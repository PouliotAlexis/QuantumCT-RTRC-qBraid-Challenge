...
class TestSaveInstanceResults:
    ...
    def test_save_instance_results_creates_file_and_header(self, csv_path):
        ...
        save_instance_results(1, 10, 100, 1.2345, csv_path)
        
        assert os.path.exists(csv_path)
        with open(csv_path, "r") as f:
            reader = list(csv.reader(f))
            assert len(reader) == 2
            assert reader[0] == ["CVRP Instance #", "# of Qubits", "# of Gate Operations", "Execution Time"]
    ...
    def test_save_instance_results_appends_data(self, csv_path, runs):
        for i in range(runs):
            save_instance_results(i, i*10, i*100, i*1.1, csv_path)
