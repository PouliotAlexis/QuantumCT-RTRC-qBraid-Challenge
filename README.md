...
│   └── bruteforce_optimizer.py    # Classical exact solver for benchmarking
├── data/
│   ├── Instance1.txt              # Solution file — instance 1
│   ├── Instance2.txt              # Solution file — instance 2
│   ├── Instance3.txt              # Solution file — instance 3
│   ├── Instance4.txt              # Solution file — instance 4
│   ├── run_results.csv            # Quantum resource metrics per run
│   └── optimal_results.csv        # Known optimal/heuristic solutions for benchmarking
├── old_readme/                    # Archived documentation versions
└── challenge/
...
- `Instance[N].txt` → routes in hackathon format
- `run_results.csv` → execution time, distance, and quantum metrics (qubits, gates, depth)
...
from utils.clustering_qaoa import solve_tsp
```
